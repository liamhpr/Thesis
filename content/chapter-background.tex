...
Our work lies at the intersetion of Natural Language Processing (NLP)
and robust Uncertainty Quantification (UQ). To rigorously address the 
problem of hallucination detection, we must first understand the 
...
\begin{equation}
    \Delta_K = \left\{ \mathcal{P} \in \mathbb{R}^K \mid 
	\forall k: \mathcal{P}_k \geq 0, \sum_{k=1}^K \mathcal{P}_k = 1
	\right\} 
    \label{eq:simplex}
\end{equation}

In standard Bayesian inference, a precise probability distribution
is a single point within this simplex. In contrast, a credal set 
represents a region of points.
...
value $P(c_k)$, we assign a lower bound $\underline{P}(c_k)$ and an
upper bound $\overline{P}(c_k)$ to each class $c_k$
\parencite{article}.
...
\begin{equation}
    \mathcal{K} = \left\{ P \in \Delta_K \mid \underline{P} (c_k) 
	\leq P(c_k) \leq \overline{P}(c_k), \sum_{k=1}^{K} P(c_k) = 1 
	\right\}
...
\begin{equation}
    H_*(\mathcal{K}) = \min_{q \in \mathcal{K}} H(q), 
	\quad H^*(\mathcal{K}) = \max_{q \in \mathcal{K}} H(q)
