...
and Uncertainty Quantification (UQ). To rigorously address the 
problem of UQ, we must first understand the 
probabilistic nature of modern language models and the limitations 
...
Language Processing (NLP), aleatoric uncertainty is highly prevalent
...
\textbf{Likelihood and Negative Log-Likelihood (NLL)} \\
...
context $\mathbf{x}$ and a target sequence $\mathbf{y}$,
...
While sequence likelihood evaluates the probability the model assigns
...
with a probability distribution $P(Y_t \mid \mathbf{y}_{<t}, \mathbf{x})$
over the vocabulary $\mathcal{V}$, the Shannon Entropy $H$ is
defined as:
\begin{equation}
    H(Y_t) = -\sum_{v \in \mathcal{V}} 
    P(y_t = v \mid \mathbf{y}_{<t}, \mathbf{x})
    \log P(y_t = v \mid \mathbf{y}_{<t}, \mathbf{x})
...
deep learning models, including LLMs, which frequently suffer from 
