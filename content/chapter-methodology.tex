...
We define the \textbf{lower probability} $\underline{P}(c_k \mid x)$ and 
the \textbf{upper probability} $\overline{P}(c_k \mid x)$  as the minimum and 
maximum probabilities observed for that cluster. 

\begin{equation}
    \underline{P}(c_k \mid x) = \min_{\tau \in \mathcal{T}_\alpha} 
	P^\tau(c_k \mid x)
    \quad \quad
    \overline{P}(c_k \mid x) = \max_{\tau \in \mathcal{T}_\alpha}
	P^\tau(c_k \mid x)
    \label{eq:cluster_bounds}
\end{equation}

These bounds define an interval 
$[\underline{P}(c_k \mid x), \overline{P}(c_k \mid x)]$ representing the model's 
...
\begin{equation}
	...
	\underline{P}(c_k \mid x) \leq P(c_k \mid x) \leq \overline{P}(c_k \mid x),
	\sum_{k = 1}^K P(c_k \mid x) = 1 \right\}
    \label{eq:credal_set}
\end{equation}
...
\begin{equation}
	...
	\left( - \sum_{k=1}^K P(c_k \mid x) \log P(c_k \mid x) \right)
