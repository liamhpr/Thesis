% !TEX root = ../main.tex
%
\chapter{Methodology: Credal Prediction via Temperature Scaling}
...
space and appplying the statistical principles of relative likelihood,
...
\begin{figure}
    \includegraphics[width=\textwidth]{drawio/.pdf}
...
\begin{enumerate}
    ...
    \item \textbf{Computation Cluster Probability Bounds}:
	...
	fluctate across the $\alpha$-cut. By finding the 
    ...
    \item \textbf{Quanifying Epistemic Uncertainty}:
...
temperature that yields the minimum NLL as $\tau^{ML}$:
...
The final set of valid temperature is formally defined as:
...
map the model's raw token outputs into discrete semantic concepts.
This requires a two-step process. Generating a diverse pool of 
candidate responses and clutsering them based on semantic equivalence.
...
lower probability bounds in the next step.
