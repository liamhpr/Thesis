...
upper and lower uncertainty bounds that serve as a signal 
for high model uncertainty.

\section{Overview of the Pipeline}
\label{sec:methodology:overview}
Our proposed methodology, which we term Credal Semantic Entropy,
works for a given input prompt $\mathbf{x}$ (a question and optional context),
...
\begin{enumerate}
	...
	Using the prompt $\mathbf{x}$, the LLM generates multiple candidate
	...
	Finally, we compute the Upper Shannon Entropy derived from these
	probability bounds. By maximizing the entropy within the credal
	set, we capture the overall model uncertainty.
	In the context of open-domain question-answering, this
	resulting scalar serves as a conservative confidence metric
	to predict the factual reliability of the generated answer.
\end{enumerate}
...
\begin{figure}
    ...
    every temperature to establish the probability intervals 
    used for UQ.}
...
For every question-answer pair $(\mathbf{x}^{(i)}, \mathbf{y}^{(i)})$ in the 
...
\begin{equation}
    ...
    \mathbf{y}_{<t}^{(i)}, \mathbf{x}^{(i)}) \right)
...
the exponentiated difference of their average NLLs:
...
we apply an $\alpha$-cut 
(Section \ref{sec:background:rel_ll_and_temp_sets:alpha_cuts}), 
retaining only those temperatures whose
...
has been established via the training dataset, we apply the methodology 
to the unseen test queries. For a given input prompt $\mathbf{x}$, the goal is
...
To evaluate the model's uncertainty, we must sample responses from 
...
pool of generated responses for the prompt $\mathbf{x}$ is the union of all 
...
$C = \{ c_1, c_2, \dots, c_K \}$ by pairwise comparing all sequences across
all valid temperatures as described in the following.
...
deemed factually equivalent in the context of the prompt $\mathbf{x}$ and are 
...
\begin{equation}
    P^\tau(C_k \mid \mathbf{x}) = 
    \frac{P^\tau(c_k \mid \mathbf{x})}{\sum_{c \in C} P^\tau(c \mid \mathbf{x})}
...
\begin{equation}
    P^\tau(c_k \mid \mathbf{x}) = \sum_{s \in c_k} P^\tau(s \mid \mathbf{x})
...
at a particular temperature $\tau$, we assign it a zero probability 
...
probability $P^\tau(s \mid \mathbf{x})$, we use the length normalized
...
We define the \textbf{lower probability} $\underline{P}(c_k \mid \mathbf{x})$ and 
the \textbf{upper probability} $\overline{P}(c_k \mid \mathbf{x})$  as the minimum and 
maximum probabilities observed for that cluster. 

\begin{equation}
    \underline{P}(c_k \mid \mathbf{x}) = \min_{\tau \in \mathcal{T}_\alpha} 
    P^\tau(C_k \mid \mathbf{x})
    \quad \quad
    \overline{P}(c_k \mid \mathbf{x}) = \max_{\tau \in \mathcal{T}_\alpha}
    P^\tau(C_k \mid \mathbf{x})
    \label{eq:cluster_bounds}
\end{equation}

These bounds define an interval 
$[\underline{P}(c_k \mid \mathbf{x}), \overline{P}(c_k \mid \mathbf{x})]$ representing the model's 
...
\begin{equation}
	...
	\underline{P}(c_k \mid \mathbf{x}) \leq P(c_k \mid \mathbf{x}) \leq \overline{P}(c_k \mid \mathbf{x}),
    \sum_{k = 1}^K P(c_k \mid \mathbf{x}) = 1 \right\}
    \label{eq:credal_set}
\end{equation}
...
To measure the uncertainty over the semantic clusters, we adapt
the credal evaluation framework from standard classification tasks,
as detailed in Section \ref{sec:background:credal_sets:uncertainty}.
By treating each semantic cluster as a distinct class, we can compute
the lower and upper bounds of the Shannon Entropy to quantify the
model's uncertainty.
% FIX:
...
our primary signal for high model uncertainty
...
\begin{equation}
    H^*(\mathcal{K}) = \max_{P \in \mathcal{K}} 
    \left( - \sum_{k=1}^K P(c_k \mid \mathbf{x}) \log P(c_k \mid \mathbf{x}) \right)
...
\begin{equation}
    H_*(\mathcal{K}) = \min_{P \in \mathcal{K}}
    \left( - \sum_{k=1}^K P(c_k \mid \mathbf{x}) \log P(c_k \mid \mathbf{x})\right)
...
This makes it ideally suited for evaluating a model's answers in a
conversational context. In such settings, our primary goal is to
bound the total predictive uncertainty to ensure safe responses,
rather than explicitly distinguish between the model's lack of
knowledge and inherent data noise.
